...
function Content() {
  return (
    <ClassContainer>
      ...
      ser tocadas, vistas e medidas são concretas, enquanto coisas que existem
      apenas como conceitos são abstratas. Por essa definição, um cachorro é
      algo concreto, que podemos ver, sentir, tocar e medir. Agora, números são
      algo abstrato. Existem em um mundo de ideias, mas não têm uma existência
      física palpável. <p />
      ...
      ignorar todas as outras características específicas de Tobi que o fazem
      único, como seu nome, horário e local de nascimento, gostos,
      ...
      diferentes graus nessa ideia. Sem muito esforço você pode perceber que até
      a própria definição do que é um cachorro é um tipo de abstração. De fato,
      ...
      <ClassBox
        content={` Um problema aritmético é algo
        como somar, subtrair, multiplicar ou dividir pares de números. Um problema
        lógico tem a ver com estabelecer se certas afirmações sobre valores
        numéricos são verdadeiras ou falsas.`}
        float="left"
      />
      ...
      <p />
      Atualmente, nós conseguimos criar <i>softwares</i> que executam em diverso
      ...
      computador e através deles conseguimos passar dados para o computador e
      ...
      A CPU é aquilo que chamamos de processador ou o cérebro do computador. Do
      ...
      repetida podemosrepresentar qualquer número que quisermos, correto? Porém,
      ...
      Dessa forma, se quisermos representar um número dentro do circuito
      ...
      determinado algarismo representa depende de sua posição. Quanto mais à
      ...
      na base binária.
      <p />
      Acredite se quiser, para um computador só existem uns e zeros. Quando eu
      digo tudo é realmente tudo! O <i>software</i> que executa em um computador
      é convertido em código binário é representado como uns e zeros. Todos os
      arquivos, textos, imagens, vídeos, documentos, gráficos são codificados
      como grandes sequências numéricasUm computador representa tudo através de
      ...
      utilizava para fazer meus trabalhos de escola, como eles são apenas
      <ClassBox
        content={` Um computador representa tudo através de números, 
        e todos esses números estão no sistema numérico binário.`}
        float="right"
      />
      números se eles na verdade contém diversas letras? Apenas muito tempo
      depois que eu entendi que cada caractere em um texto é representado como
      um número, e que existem tabelas de referência, como tabela ASCII que
      ...
      é possível que todos aqueles pixels que formam as imagens na minha tela
      fossem apenas números? Apenas depois de bastante estudo que eu fui capaz
      ...
      numéricas. São apenas números binários que precisam ser interpretados pelo{" "}
      <i>software</i> e transformados em sinais enviados para os dispositivos de
      saída do computador.
      <p />
      Um digito binário qualquer em um desses arquivos é chamado de bit (
      <i>binary digit</i>). O átomo da informação digital em nossa era. Se
