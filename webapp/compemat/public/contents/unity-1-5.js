...
function Content() {
  return (
    <ClassContainer>
      ...
      />
      <p />
      Agora que sabemos como acessar a memória através de variáveis, e
      entendemos como podemos criar expressões para realizar operações diversas,
      vamos aprender como podemos fazer com que o nosso código se comunique com
      o mundo exterior.
      <p />
      ...
      programa estar correto. Obvio que se você pretende continuar no caminho
      para se tornar um desenvolvedor, em algum momento você vai ter que abrir
      mão do conforto e colocar a mão na massa. Mas não há mal de facilitar a
      nossa vida enquanto estamos aprendendo, não é mesmo?
      <p />
      ...
      <p />
       As IDEs contém um editor de texto onde um dev (developer, desenvolvedor
      ou desenvolvedora de software) pode digitar código. Um conjunto de
      ...
      desenvolvidas por mim e estarão disponíveis a partir do seu código. Quando
      ...
      colocar dados em um formato conhecido como JSON, sendo um formato muito
      utilizado atualmente para representar dados compatível com a maneira como
      o JavaScript armazena dados complexos em suas variáveis. Nesse caso
      teremos um JSON com algumas chaves de entrada nomeadas seguidos de
      valores. Exemplo:
      <p />
      <ClassCodeEditor
        code={`{\n   "nome":"Hermano"\n}`}
      ...
      uma única chave chamada nome e cujo valor é Hermano. Note que tanto o nome
      da chave quanto o valor são strings (cadeias de caracteres), por isso
      precisam estar entre aspas, neste caso duplas.
      <p />
      ...
      />
      <p />
      O comando de input terá o seguinte forma. Você usará sempre um comando de
      atribuição com declaração de variável. Você pode dar qualquer nome para
      variável, contanto que o nome esteja de acordo com as regras, mas vai ser
      mais fácil se o nome da variável for igual ao nome da entrada que você
      especificou no JSON. Após o sinal de igual (=) você irá colocar o nome
      input, e entre parênteses e chaves irá colocar o nome da entrada cujo
      valor você quer obter. Se você executar o código acima, clicando no botão
      com símbolo de play azul, não vai ver nenhum resultado, pois seu programa
      apenas faz a entrada e não faz nenhuma saída. Para ver o comportamento
      completo, é necessário completar nosso código.
      <p />
      <ClassCodeEditor
      ...
      />
      <p />
      Agora sim, temos um programa completo que faz as três coisas que um
      software precisa fazer. A primeira é a entrada de dados, no caso o input
      nome armazenado em uma variável de mesmo nome. Após isso criamos uma nova
      variável chamada mensagem. Essa variável é atribuída ao resultado de uma
      expressão. Essa é uma expressão muito legal, pois ela faz a concatenação
      de strings. Concatenar envolve combinar duas cadeias de caracteres e dar
      origem a uma nova que possui o texto das duas. Nós utilizamos o operador +
      que, em geral, serve para somar números, porém, em JavaScript e em
      diversas outras linguagens esse operador também pode ser utilizados em
      pares de cadeias de caracteres para dar origem a strings concatenadas.
      <p />
      Finalmente, na terceira linha temos o comando output. O comando output
      permitirá que a gente faça a saída de dados de forma que vamos conseguir
      ver os dados que nosso programa gerou. Colocaremos entre parênteses uma
      variável ou literal ou expressão que dê origem ou contenha o dado que
      queremos visualizar na saída.
      <p />
      No caso do nosso programa, a saída do mesmo será:
      <p />
      Olá Hermano
      <p />
      Sugiro que experimente executar o código e ver o resultado. Faça
      alterações na entrada no painel à direita, coloque seu nome. Você também
      pode mudar a mensagem, ao invés de olá, coloque um bom dia, ou boa noite,
      ou mesmo uma mensagem engraçada. Divirta-se com o código!
    </ClassContainer>
  );
}
