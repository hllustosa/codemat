...
function Content() {
  return (
    <ClassContainer>
      ...
      computador é composto pelos chamados pentes de memória, compostos por
      módulos com um grande número de capacitores microscópicos capazes de
      armazenar carga. A existência ou não da carga em um desses capacitores
      ...
      buracos feitos com bases em forma geométricos. Para encaixar um dado (que
      ...
      <ul>
        ...
        <li>
          ...
          &amp;. As exceções são &lowbar; (underscore) e &dollar; (cifrão).
        </li>
        <li>
          Nomes de variáveis não podem começar com números, apenas letras,
          &lowbar; e &dollar;.
        </li>
        <li>
      ...
      é inválido, pois começa com um número e não com uma letra, &lowbar; ou
      &dollar;.
      <p />
      ...
      texto que representa o dado que queremos colocar em nosso código.
      <p />
      Os literais numéricos são fáceis de identificar. São simplesmente números
      que escrevemos no código. As literais booleanas ou lógicas são as palavras
      ...
      cadeia de caractere no seu código pode possuir qualquer texto), precisam
      estar delimitadas por aspas simples ou aspas duplas. As literais de cadeia
      de caracteres precisam ser limitadas no próprio código para que o
      interpretador saiba onde elas começam, onde elas terminam e onde começa o
      restante do código.
      <p />
      <ClassSectionTitle title="Comando de Atribuição" />
      Agora que entendemos o que é uma variável e o que um literal, podemos
      aprender a executar nosso primeiro comando em JavaScript, o comando de
      atribuição. Um comando de atribuição é utilizado para ser possível
      ...
      />
      <p />
      Um comando de atribuição pode começar ou não com a palavra reservada var.
      Essa palavra é utilizada para indicar que estamos declarando a variável,
      isto é, utilizamos ela pela primeira vez. Podemos colocar em nosso código
      quantos comandos de atribuição quisermos, porém, o primeiro comando de
      atribuição para cada variável em nosso código deve obrigatoriamente conter
      a palavra var no começo.
      <p />
      Na sequência teremos o nome da variável (que às vezes é chamado de
      identificador). Entre a palavra var e o nome da variável deve
      obrigatoriamente existir um espaço em branco. Após o nome da variável
      iremos utilizar o sinal de igual (=), que nesse caso é chamado de operador
      de atribuição. Esse sinal de igual é utilizado para indicar que algum
      valor deve ser armazenado na variável.
      <p />
      ...
      além das regras da linguagem, as boas práticas. Essas boas práticas são
      conjuntos de regras que variam conforme a linguagem de programação,
      ambiente e mesmo organizações diferentes. Muitas vezes as regras são
      subjetivas ou mesmo questionáveis. Porém, algumas práticas mais básicas
      são amplamente aceitas.
      <p />
      Uma delas diz respeito ao nome de variáveis. Sempre pense com carinho no
      nome da variável ao cria-la. Dê às suas variáveis nomes significativos,
      que façam menção a qual dado ela está armazenando. Isso vai fazer com que
      o seu código fique mais legível e fácil de entender por parte de outros
      programadores e por você mesmo no futuro.
      <p />
      Vamos a alguns exemplos de comandos de atribuição validos que podemos ter
      em nosso código:
      <p />
      <ClassCodeEditor
      ...
      />
      <p />
      Esse exemplo contém um comando de atribuição com da variável cujo nome é
      variavelAindaNaoDeclarada e que terá a partir da execução dessa linha o
      valor 42. Esse comando irá falhar se em uma linha de código anterior você
      já tiver declarado uma variável cujo nome já é variavelAindaDeclarada.
      <p />
      <ClassCodeEditor code={`variavelJaDeclarada = 42;`} height="55px" />
      <p />
      Esse exemplo contém um comando de atribuição com da variável cujo nome é
      variavelJaDeclarada . Por não conter a palavra a var, esse comando só pode
      ser utilizado se em alguma linha de código anterior a variável tiver sido
      declarada com a palavra var na frente.
      <p />
      <ClassCodeEditor
      ...
      uma variável do tipo string armazenando o valor: "Texto Literal".
      ...
      atribuição. Uma expressão é algo que será avaliado pelo interpretador, que
      irá dar origem a um valor. Se colocarmos essa expressão à direita do sinal
      de igual em um comando de atribuição, o resultado dessa expressão será
      ...
      utilizar diversos operadores aritméticos, tais como:
      <p />
      <ul>
        <li>
          <b>+</b> soma
      ...
      literais podemos utilizar identificadores de variáveis. Por exemplo:
      <p />
      <ClassCodeEditor
      ...
      <p />
      No exemplo de código, mesmo operador de soma estando mais à esquerda que o
      operador de divisão, pelo fato do operador de divisão possuir maior
      ...
      />
      <p />
      Neste caso, os primeiros somam-se valor1 e valor2, obtendo-se como
      resultado o valor 19, e depois dividimos este valor por 2,0, obtendo o
      valor 9,5.
      <p />
      ...
      língua inglesa e em suas convenções. Em inglês utilizamos o ponto(.) ao
      invés da vírgula para separar a parte inteira da parte fracionada de um
      número. Portanto, sempre que ver um ponto(.) em um número no código, saiba
      que este é o separador de casas decimais.
      <p />
      Outro ponto interessante a notar que por padrão os números que escrevemos
      estão na base 10. A base com a qual estamos mais acostumados. Podemos
      utilizar a base 10 sem problemas em nosso código, pois o interpretador irá
      transformar nosso texto em uma representação binária utilizada pelo
      computador automaticamente. Existem maneiras de especificar literais em
      outras bases numéricas, mas este é um tópico que foge do escopo do nosso
      curso.
    </ClassContainer>
  );
}
